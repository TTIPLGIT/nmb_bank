(function($){
	...
	if (onlyEndNodes == true) {
		...
		var input_nodes = activeGroup.find('input:checkbox.hummingbird-end-node:checked');
	    } else {
		var input_nodes = tree.find('input:checkbox.hummingbird-end-node:checked');
	    }
	    ...
	    if (onlyParents == true){
		tree.find('input:checkbox:checked:not(.hummingbird-end-node)').each(function() {
		    list.text.push($(this).parent("label").parent("li").text());
		    list.id.push($(this).attr("id"));
		    list.dataid.push($(this).attr("data-id"));
		});
