...
class OfficeConverterController
{
    /* @var string /
    private $file;
    /* @var string /
    ...
    public function convertTo($filename)
    {
        ...
        $outdir = public_path().'/documents/pdfview/';
    ...
    protected function setup($tempPath, $bin, $prefixExecWithExportHome)
    {
        ...
        if (!array_key_exists($extension, $this->getAllowedConverter())) {
            throw ('Input file extension not supported -- '.$extension);
        }
    ...
    private function exec($cmd, $input = '')
    {
        ...
        if (false === $process) {
            throw ('Cannot obtain ressource for process to convert file');
        }
