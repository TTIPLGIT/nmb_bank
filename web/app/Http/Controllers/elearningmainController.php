...
class elearningmainController extends BaseController
{
    ...
    public function courseOverview(Request $request, $id)
    {
        ...
        $menunew = BaseController::FillMenu();
        //dd($menunew);
        $menus = $this->FillMenu();
        $screens = $menus['screens'];
        $modules = $menus['modules'];

        return view('elearning.courseOverview', compact('courseDetails', 'courseContents', 'classOrder', 'enrolled', 'modules', 'screens', 'menus', 'user_id'));
    }
    ...
    public function takeCourse(Request $request, $id)
    {
        ...
        //$classResourse = DB::select("SELECT * FROM elearning_classes where class_id= $id");
        // dd($selected_class);
    ...
    public function addWishList(Request $request)
    {
        ...
        $isWishListed = DB::select("SELECT * FROM elearning_wishlist WHERE user_id = $userId AND course_id = $courseId and active_flag='0' ");
        ...
        if (empty($isWishListed)) {
            ...
            $cart_data = DB::select("SELECT id FROM elearning_cart WHERE user_id = $userId AND course_id = $courseId");
            $this->WriteFileLog($cart_data);
            if (!empty($cart_data)) {
                $id  = $cart_data[0]->id;
            ...
            $data = DB::table('elearning_wishlist')
