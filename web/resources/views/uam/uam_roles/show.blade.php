...
@section('content') <div class="main-content module_space">
  ...
  <section class="section">
    ...
    <div class="section-body mt-1">
      <h5 class="heading_align"  style="color:darkblue">Role Show</h5>

      {{ Breadcrumbs::render('uam_roles.show',$rows[0]['role_id']) }}
      ...
      <div class="row">
        ...
        <div class="col-12">
          ...
          <div class="card">
            <div class="card-body">
              <form class="form-horizontal" method="post" name="uam_roles" id="uam_roles" onsubmit="return validateForm()" action="{{ route('uam_roles.update',$rows[0]['role_id']) }}">
                ...
                <div class="row">
                  <div class="col-md-6">
                    <div class="form-group">
                      ...
                      <input class="form-control" type="text" id="role_name" name="role_name" placeholder="Enter Role Name" value="{{ $rows[0]['role_name'] }}" autocomplete="off" disabled="">
                    </div>
                    @error('role_name')
                ...
                <div class="col-md-12">
                  <div id="treeview_container" class="hummingbird-treeview well h-scroll-large">
                    ...
                    <ul id="treeview" class="hummingbird-base">
                      ...
                      <!-- Module 1 -->
                      <li>
                        <i class="fa fa-plus"></i> <label> <input id="node-{{ $parent_module_data_value['module_id'] }}" data-id="{{ $parent_module_data_value['module_id'] }}" type="checkbox" module="{{ $parent_module_data_value['module_id'] }}"> {{ $parent_module_data_value['module_name'] }} </label>
                        <ul>
                          @if($module_data !="")

                          @foreach ($module_data as $key => $module_data_value)
                          @if($parent_module_data_value['module_id'] == $module_data_value['parent_module_id'])




                          <li><i class="fa fa-plus"></i> <label> <input id="node-{{$module_data_value['parent_module_id'] }}-{{$module_data_value['module_id'] }}" data-id="{{$module_data_value['parent_module_id'] }}-{{$module_data_value['module_id']}}" module="{{$module_data_value['parent_module_id'] }}" type="checkbox"> {{$module_data_value['module_name'] }}</label>
                            <!-- Module 2 -->
                            <ul>
                              @if($module_data !="")

                              @foreach ($module_data as $key => $module_data_value_two)



                              @if($module_data_value_two['parent_module_id'] == $module_data_value['module_id'])


                              <li><i class="fa fa-plus"></i> <label> <input id="node-{{$module_data_value_two['parent_module_id'] }}-{{$module_data_value['module_id'] }}" data-id="{{$module_data_value_two['parent_module_id'] }}-{{$module_data_value['module_id'] }}" module="{{$module_data_value_two['parent_module_id'] }}" type="checkbox"> {{$module_data_value_two['module_name'] }}</label>
                                <!-- Module 3 -->
                                <ul>
                                  @if($module_data !="")
                                  @foreach ($module_data as $key => $module_data_value_three)

                                  @if($module_data_value_three['parent_module_id'] == $module_data_value_two['module_id'])

                                  <li><i class="fa fa-plus"></i> <label> <input id="node-{{$module_data_value_three['parent_module_id'] }}-{{$module_data_value_two['module_id']}}" data-id="{{$module_data_value_three['parent_module_id'] }}-{{$module_data_value_two['module_id'] }}" module="{{$module_data_value_three['parent_module_id'] }}" type="checkbox"> {{$module_data_value_three['module_name'] }}</label>

                                    <!-- Module 4 -->
                                    <ul>
                                      @if($module_data !="")
                                      @foreach ($module_data as $key => $module_data_value_four)
                                      @if($module_data_value_four['parent_module_id'] == $module_data_value_three['module_id'])

                                      <li><i class="fa fa-plus"></i> <label> <input id="node-{{$module_data_value_four['parent_module_id'] }}-{{$module_data_value_three['module_id'] }}" data-id="{{$module_data_value_four['parent_module_id'] }}-{{$module_data_value_three['module_id'] }}" module="{{$module_data_value_two['parent_module_id'] }}" type="checkbox"> {{$module_data_value_four['module_name'] }}</label>
                                        <ul>

                                          @if($screens_data !="")

                                          @foreach ($screens_data as $key => $screen_data_value_four)
                                          @if($module_data_value_four['module_id'] == $screen_data_value_four['module_id'])
                                          <li> <label><input id="node-{{$screen_data_value_four['module_id'] }}-{{$screen_data_value_four['screen_id'] }}" data-id="{{$screen_data_value_four['module_id'] }}:{{$screen_data_value_four['screen_id'] }}" type="checkbox"> {{$screen_data_value_four['screen_name'] }} </label>
                                            <ul style="display:inline-flex;float:right">

                                              @if($permissions_data !="")

                                              @foreach ($permissions_data as $key => $permissions_data_value)
                                              @if($screen_data_value_four['screen_id']== $permissions_data_value['screen_id'])
                                              <li> <label><input class="hummingbird-end-node" data-id="node1-{{$permissions_data_value['id'] }}-{{$screen_data_value_four['module_id'] }}-{{$screen_data_value_four['screen_id']}}" id="{{$screen_data_value_four['module_id'] }}:{{$permissions_data_value['screen_id'] }}" type="checkbox"> {{$permissions_data_value['permission'] }} </label></li>
                                    ...
                                    <ul>
                                      ...
                                      @if($screens_data !="")
                                      @foreach ($screens_data as $key => $screen_data_value_three)
                                      @if($module_data_value_three['module_id'] == $screen_data_value_three['module_id'])
                                      <li> <label><input id="node-{{$screen_data_value_three['module_id'] }}-{{$screen_data_value_three['screen_id'] }}" data-id="{{$screen_data_value_three['module_id'] }}:{{$screen_data_value_three['screen_id'] }}" type="checkbox"> {{$screen_data_value_three['screen_name'] }} </label>
                                        <ul style="display:inline-flex;float:right">

                                          @if($permissions_data !="")

                                          @foreach ($permissions_data as $key => $permissions_data_value)
                                          @if($screen_data_value_three['screen_id']== $permissions_data_value['screen_id'])
                                          <li> <label><input class="hummingbird-end-node" data-id="node1-{{$permissions_data_value['id'] }}-{{$screen_data_value_three['module_id'] }}-{{$screen_data_value_three['screen_id']}}" id="{{$screen_data_value_three['module_id'] }}:{{$permissions_data_value['screen_id'] }}" type="checkbox"> {{$permissions_data_value['permission'] }} </label></li>
                                ...
                                <ul>
                                  ...
                                  @if($screens_data !="")
                                  @foreach ($screens_data as $key => $screen_data_value_two)
                                  @if($module_data_value_two['module_id'] == $screen_data_value_two['module_id'])
                                  <li> <label><input id="node-{{$screen_data_value_two['module_id'] }}-{{$screen_data_value_two['screen_id'] }}" data-id="{{$screen_data_value_two['module_id'] }}:{{$screen_data_value_two['screen_id'] }}" type="checkbox"> {{$screen_data_value_two['screen_name'] }} </label>
                                    <ul style="display:inline-flex;float:right">

                                      @if($permissions_data !="")

                                      @foreach ($permissions_data as $key => $permissions_data_value)
                                      @if($screen_data_value_two['screen_id']== $permissions_data_value['screen_id'])
                                      <li> <label><input class="hummingbird-end-node" data-id="node1-{{$permissions_data_value['id'] }}-{{$screen_data_value_two['module_id']}}-{{$screen_data_value_two['screen_id']}}" id="{{$screen_data_value_two['module_id'] }}:{{$permissions_data_value['screen_id'] }}" type="checkbox"> {{$permissions_data_value['permission'] }} </label></li>
                            ...
                            <ul>
                              ...
                              <li> <label><input id="node-{{$screen_data_value['module_id'] }}-{{$screen_data_value['screen_id'] }}" data-id="{{$screen_data_value['module_id'] }}:{{$screen_data_value['screen_id'] }}" type="checkbox"> {{$screen_data_value['screen_name'] }} </label>
                                <ul style="display:inline-flex;float:right">
                ...
                <div class="row text-center">
                  <div class="col-md-12">
                    ...
                    <a class="btn btn-danger" href="{{ route('uam_roles.index') }}"><i class="fa fa-times"></i> Cancel </a>&nbsp;
                  </div>
                </div>
            </div>
            </form>
