    ...
    <section class="section">
        <div class="section-body mt-1">
            <div class="row">
                ...
                $id = $quizId;
                @endphp
                <form id="quizForm" class="row" action="{{ route('class.quizstore', $quizId) }}" method="post">
                        ...
                        <div class="card noShadow px-2 py-3">
                            ...
                            <div class="card-body">
                                <div class="mcqChoicesContainer">
                                    ...
                                    @php
                                    $option = chr(64 + $loop->iteration); // A, B, C, D
                                    @endphp

                                    <div class="custom-control custom-checkbox">
                                        <input type="checkbox"
                                            class="custom-control-input mcq_{{ $questionDetail->question_id }}"
                                            id="customCheck_{{ $questionDetail->question_id }}_{{ $option }}"
                                            value="{{ $option }}" autocomplete="off">

                                        <label class="custom-control-label"
                                            for="customCheck_{{ $questionDetail->question_id }}_{{ $option }}">
                                            {{ $option }}. {{ $choice }}
                                        </label>
                                    </div>
                                    @endforeach
...
function answerArray(e) {
    ...
    for (let questionArray of questionsArray) {
        if (questionArray[1] == "boolean") {
            ...
            for (let choice of choices) {
                if (choice.checked) {
                    selections.push(choice.value.toUpperCase());
                }
            }
...
function quizResult(e) {
    ...
    document.querySelector('.totalScoreBody .circle').style.backgroundImage =
        `conic-gradient(from 180deg, #28a745 ${Math.floor((quizResultData.totalPointsEarned*100)/quizResultData.totalAvailablePoints)}%, #fff 0)`;
    document.querySelector('.totalScoreBody .inner').innerHTML =
        `${quizResultData.totalPointsEarned}/${quizResultData.totalAvailablePoints}`;
    for (let answerDetail of quizResultData.answerDetails) {
        if (answerDetail.questionType == "boolean") {
            document.querySelector(`#customRadio_${answerDetail.questionId}_true + label`).addEventListener('click',
                disabled);
            document.querySelector(`#customRadio_${answerDetail.questionId}_false + label`).addEventListener('click',
                disabled);
            if (answerDetail.answerStatus == "1" && answerDetail.answerGiven == "True") {
                document.querySelector(`#customRadio_${answerDetail.questionId}_true + label`).innerHTML =
                    "True&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&#9989;";
                ...
                document.querySelector(`#customRadio_${answerDetail.questionId}_false + label`).innerHTML =
                    "False&nbsp;&nbsp;&nbsp;&nbsp;&#9989;";
            ...
            if (answerDetail.answerStatus == "none") {
                ...
                document.querySelector(`#shortResultContainer_${answerDetail.questionId}`).classList.add(
                    'shortPartial');
            ...
            for (let i = 0; i < options.length; i++) {
                document.querySelectorAll(`.mcq_${answerDetail.questionId} + label`)[i].addEventListener('click',
                    disabled);
            }
