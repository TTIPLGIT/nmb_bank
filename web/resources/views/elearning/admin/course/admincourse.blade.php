...
<script>
    ...
    $select.on('select2:select', function(e) {
        let selectedValue = String(e.params.data.id).toLowerCase();

        if (selectedValue === 'all') {
            // Select ALL option values
            ...
            // Immediately update checkboxes for "All"
            updateDropdownCheckboxes();
        } else {
            // If any other option is selected AND "All" is selected, keep all selected
    ...
    $select.on('select2:unselect', function(e) {
        let unselectedValue = String(e.params.data.id).toLowerCase();
    ...
    // Function to visually check already-selected items
    function updateDropdownCheckboxes() {
        let selected = $select.val() || [];
        $('.select2-results__option').each(function() {
            let val = $(this).data('select2-id');
    ...
    // Keep checkboxes correct when dropdown opens
